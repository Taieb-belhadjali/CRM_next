...
export async function POST(request) {
  try {
    ...
    if (!firstName || !lastName) return withCors(Response.json({ error: "firstName and lastName are required" }, { status: 400 }));
    await dbConnect();
    const contact = await Contact.create({
      firstName: firstName.trim(), lastName: lastName.trim(),
      jobTitle: jobTitle?.trim(), email: email?.trim().toLowerCase(),
      phone: phone?.trim(), address: address?.trim(),
      account: account || undefined, owner: auth.sub,
    });
    const populated = await contact.populate([{ path: "account", select: "name" }, { path: "owner", select: "name email" }]);
    logActivity({ auth, request, action: "contact_create", entity: "contact", entityId: contact._id, entityLabel: `${firstName.trim()} ${lastName.trim()}` });
